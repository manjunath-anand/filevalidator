...
import org.anz.codechallenge.validators.{FieldValidator, FileNameValidator}
...
class FieldValidatorTest extends AnyFunSuite with BeforeAndAfterAll {
  ...
  test("test field value scenario"){
    ...
    val recordCountValidator = new FieldValidator(inputMetadata,file_schema,tagFile, datadf)
